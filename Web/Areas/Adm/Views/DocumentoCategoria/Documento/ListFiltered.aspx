...
<div id="container">
	<div class="minicontainer mc-l">
        ...
        <div class="alerta-azul" style="background-image:url(/Content/CSS/Back/icons/information.png);">Resultados para filtros de b&uacute;squeda aplicados (<%=ViewData["FiltroAplicado"] %>). <a href="<%=Url.Action("List","Documento") %>" style="cursor:pointer;text-decoration:underline;">Mostrar todos los documentos</a>.</div>
...
<script type="text/javascript">
    $(document).ready(function ()
    {
        ...
        $('#filtroCategoria').val(<%=ViewData["Filter_Categoria"] %>);
    });
...
<script type="text/javascript">
    function SetPage(p)
    {
        window.location = '<%=Url.Action("ListFiltered","Documento") %>?filtro=<%=ViewData["FiltroAplicado"] %>&nombre=<%=ViewData["Filter_Nombre"] %>&seccion=<%=ViewData["Filter_Seccion"] %>&categoria=<%=ViewData["Filter_Categoria"] %>&page=' + p + '&perpage=<%=ViewData["Paginator_PerPage"] %>';
    }
    function ChangeMaxPageRows(r)
    {
        window.location = '<%=Url.Action("ListFiltered","Documento") %>?filtro=<%=ViewData["FiltroAplicado"] %>&nombre=<%=ViewData["Filter_Nombre"] %>&seccion=<%=ViewData["Filter_Seccion"] %>&categoria=<%=ViewData["Filter_Categoria"] %>&page=1&perpage=' + r;
    }
</script>
</asp:Content>
