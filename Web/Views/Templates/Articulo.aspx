...
<div class="wrapper">
    ...
    <div class="content <%=((Camarco.Model.Seccion)ViewData["seccion"]).Color %>">
        ...
        <% Html.RenderPartial("~/Views/Shared/BreadCrumbExtended.ascx", ((Camarco.Model.Seccion)ViewData["seccion"])); %>

      <h2 class="title"><%=Html.Encode(((Camarco.Model.Articulo)ViewData["resource"]).Titulo.ToUpper()) %></h2>
      <!-- MODULO ARTICULO -->
      <div class="articulo">
        
        <% Html.RenderPartial("~/Views/Shared/ArticuloMultimedia.ascx", ((Camarco.Model.Articulo)ViewData["resource"])); %>
        <p class="summary"<%=((Camarco.Model.Articulo)ViewData["resource"]).DescripcionCorta %></p>
        <%=((Camarco.Model.Articulo)ViewData["resource"]).DescripcionLarga%>
      </div>
      <!-- FIN MODULO ARTICULO -->
      <!-- PDF WARNING -->
      <% 
          if (((Camarco.Model.Articulo)ViewData["resource"]).Archivos.Count > 0)
          { 
        %>
    ...
    <div class="resultados">
        ...
        <% 
              ...
              foreach (Camarco.Model.ArticuloArchivo sa in ((Camarco.Model.Articulo)ViewData["resource"]).Archivos)
              {
                  string type = "";
